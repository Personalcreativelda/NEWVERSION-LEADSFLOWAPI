...
export class LeadsService {
  ...
  private normalizeLeadInput(raw: any, defaultSource?: string) {
    ...
    return {
      ...
      status: (raw?.status || raw?.situacao || 'novo').toLowerCase().trim() === 'new' ? 'novo' : (raw?.status || raw?.situacao || 'novo').toLowerCase().trim(),
      tags,
      notes,
  ...
  async create(data: any, userId: string) {
    ...
    const result = await query(
      ...
      [
        ...
        data.position,
        data.source,
        (data.status || 'novo').toLowerCase().trim() === 'new' ? 'novo' : (data.status || 'novo').toLowerCase().trim(),
        data.tags || [],
        data.notes,
        data.custom_fields || {},
        dealValue,
