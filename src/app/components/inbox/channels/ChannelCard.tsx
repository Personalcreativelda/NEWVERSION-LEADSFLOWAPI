...
import { RefreshCw, Trash2, Settings, CheckCircle2, XCircle, Loader2, MessageCircle, ExternalLink } from 'lucide-react';
...
export function ChannelCard({ channel, onEdit, onDelete, onSync, loading }: ChannelCardProps) {
        ...
        >
            ...
            >
                <div className="flex items-center gap-3">
                    <div 
                        className={`w-10 h-10 rounded-lg flex items-center justify-center ${channel.type.includes('whatsapp') ? 'bg-green-500 text-white' : ''}`}
                        style={!channel.type.includes('whatsapp') ? { backgroundColor: 'hsl(var(--muted))' } : {}}
                    >
                        {typeof getIcon(channel.type) === 'string' ? (
                            <span className="text-xl">{getIcon(channel.type)}</span>
                        ) : (
                            getIcon(channel.type)
                        )}
                    </div>
                    <div>
                        <h4 className="font-semibold leading-tight flex items-center gap-2" style={{ color: 'hsl(var(--foreground))' }}>
                            {channel.name}
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-full text-xs" style={{ 
                        ...
                        {channel.type === 'whatsapp' && (
                            <>
                                <div className="flex items-center gap-2 text-sm">
                                    ...
                                    <span style={{ color: 'hsl(var(--muted-foreground))' }}>Evolution API</span>
                                </div>
                                {channel.credentials.instance_name && (
                                    <div className="text-xs" style={{ color: 'hsl(var(--foreground))' }}>
                                        <span style={{ color: 'hsl(var(--muted-foreground))' }}>Instância atual: </span>
